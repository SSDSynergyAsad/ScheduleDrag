...
export class SelectContainerComponent implements AfterViewInit, OnDestroy, AfterContentInit {
  ...
  ngAfterViewInit() {
    if (isPlatformBrowser(this.platformId)) {
      ...
      console.log("🚀 ~ file: select-container.component.ts ~ line 162 ~ SelectContainerComponent ~ ngAfterViewInit ~ mousemove$", mousemove$);
  ...
  selectAll() {
    this.$selectableItems.forEach((item) => {
      console.log("🚀 ~ file: select-container.component.ts ~ line 272 ~ SelectContainerComponent ~ this.$selectableItems.forEach ~ item", item)
